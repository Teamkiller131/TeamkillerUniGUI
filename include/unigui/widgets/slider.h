...
Slider<T>::Slider(std::string name, std::string label, T value, T min, T max)
        : ValueWidget<T>(std::move(name), value)
        , label_(std::move(label))
        , min_(min)
        , max_(max) {}

template <typename T> void Slider<T>::SetRange(T min, T max) {
    min_ = min;
    max_ = max;
}
template <typename T> const char* Slider<T>::GetFormat() const {
    return format_;
}
template <typename T> void Slider<T>::SetFormat(const char* fmt) {
    format_ = fmt;
}
} // namespace unigui
