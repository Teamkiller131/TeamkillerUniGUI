...
public:
    static void Show(const char* id, std::vector<ContextMenuItem> items) {
        if (ImGui::BeginPopupContextItem(id)) {
            for(auto& item : items) {
                if(item.separator) ImGui::Separator();
                else if(ImGui::MenuItem(item.label.c_str()) && item.action) item.action();
            }
            ImGui::EndPopup();
        }
    }
    static void ShowWindow(const char* id, std::vector<ContextMenuItem> items) {
        if (ImGui::BeginPopupContextWindow(id, ImGuiPopupFlags_MouseButtonRight | ImGuiPopupFlags_NoOpenOverExistingPopup)) {
            for(auto& item : items) {
                if(item.separator) ImGui::Separator();
                else if(ImGui::MenuItem(item.label.c_str()) && item.action) item.action();
            }
