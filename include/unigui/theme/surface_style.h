...
inline const std::array<SurfaceStyle, 5>& AllSurfaceStyles() {
    static const std::array<SurfaceStyle, 5> kAll = {SurfaceStyle::Solid, SurfaceStyle::Glass,
                                                     SurfaceStyle::Frosted, SurfaceStyle::Acrylic,
                                                     SurfaceStyle::Minimal};
    return kAll;
}

namespace detail {
inline void MulAlpha(ImVec4& c, float m) {
    c.w = Clamp01(c.w * m);
}
...
inline SurfaceStyle ActiveSurfaceStyle() {
    return detail::g_active_surface;
}
