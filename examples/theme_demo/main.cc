...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (a == "--interval" && i+1 < argc) switchInterval = std::atoi(argv[++i]);
        else if (a == "--manual") autoCycle = false;
    }

    unigui::AppConfig cfg; cfg.width = 800; cfg.height = 500;
    ...
    while (!unigui::ShouldClose()) {
        ...
        ImGui::Text("Current Theme: %s (%d/%d)",
                    names[currentTheme].c_str(), currentTheme + 1, (int)names.size());
        ...
        static unigui::Button b1("tb1", "Primary");  b1.SetColorVariant(unigui::Button::Primary);
        static unigui::Button b2("tb2", "Danger");   b2.SetColorVariant(unigui::Button::Danger);
        static unigui::Button b3("tb3", "Success");  b3.SetColorVariant(unigui::Button::Success);
        b1.Render(); ImGui::SameLine(); b2.Render(); ImGui::SameLine(); b3.Render();
        ...
        ImGui::SameLine(60); ImGui::Text("Count:"); ImGui::SameLine(); cnt.Render();

        ImGui::Spacing();
        ImGui::Text("Animated ProgressBar:");
        static unigui::ProgressBar pb("tpb", 0.f);
        static float pbVal = 0.f;
        pbVal += 0.008f; if (pbVal > 1.f) pbVal = 0.f;
