...
- **Trading-model performance benchmarks (Horizon 4)**: new `tests/trading/bench_test.cc` exercises the header-only models under high update rates — `OrderBook` over 200k price deltas (with inside-market reads) and 5k full 100-level snapshot rebuilds, plus `OhlcSeries` folding 1M ticks into a rolling bar window and 1k per-frame ImPlot column extractions. Each carries a generous regression floor (guards against order-of-magnitude slowdowns in the DOM/blotter streaming path). Header-only, headless, always built.
