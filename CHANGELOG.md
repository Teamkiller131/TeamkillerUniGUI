...
### Fixed
- **Warnings-clean under `-Werror` (Horizon 1)**: the whole tree — library, tests, and examples — now builds clean with `UNIGUI_WARNINGS_AS_ERRORS=ON`. Removed stray unused variables/parameters from the `widget_base`/`treeview`/`style_scope` tests, and added `-Wno-missing-field-initializers` to `cmake/CompilerWarnings.cmake` (GCC/Clang): `-Wextra`'s missing-field-initializers warning conflicts with the project's clang-tidy `readability-redundant-member-init` policy (idiomatic partial aggregate initialisation is used throughout), so it is suppressed rather than fought. All 895 tests pass with warnings-as-errors on.
