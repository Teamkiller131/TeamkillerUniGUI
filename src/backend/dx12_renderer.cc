...
static void SrvDescriptorFree(ImGui_ImplDX12_InitInfo*,
    ...
    ID3D12DescriptorHeap** outRtvHeap, ID3D12DescriptorHeap** outSrvHeap)
{
    ...
    if (FAILED(hr)) { std::fprintf(stderr, "[unigui] DX12: CreateDXGIFactory2 failed: 0x%lx\n", (unsigned long)hr); return false; }
    ...
    if (FAILED(swap1.As(&swap))) { std::fprintf(stderr, "[unigui] DX12: IDXGISwapChain3 query failed\n"); return false; }
    ...
    std::fprintf(stderr, "[unigui] DX12: device + swapchain ready (%ux%u), renderer=imgui_impl_dx12\n", width, height);
...
void DX12Renderer::CleanupRenderTargets()
{
    for (int i = 0; i < kNumBackBuffers; ++i)
    {
        if (backBuffers_[i]) { backBuffers_[i]->Release(); backBuffers_[i] = nullptr; }
        rtvHandlePtr_[i] = 0;
    }
}
...
void DX12Renderer::FlushGpu()
{
    ...
    for (auto& f : frames_) f.fenceValue = 0;
}
...
bool DX12Renderer::Init(ImGuiContext* context)
{
    ...
    {
        if (FAILED(device_->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&frames_[i].alloc))))
        {
    ...
    if (FAILED(device_->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, frames_[0].alloc, nullptr, IID_PPV_ARGS(&cmd))))
    {
...
{
    if (!initialized_ || !swapchain_ || w <= 0 || h <= 0) return false;
    FlushGpu();
    CleanupRenderTargets();
