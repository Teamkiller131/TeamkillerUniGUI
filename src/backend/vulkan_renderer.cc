...
bool VulkanRenderer::Impl::CreateInstance()
{
    ...
    if (platform) platform->GetVulkanInstanceExtensions(exts);
    bool hasSurface = false;
    for (const char* e : exts)
        if (std::strcmp(e, VK_KHR_SURFACE_EXTENSION_NAME) == 0) { hasSurface = true; break; }
    if (!hasSurface) exts.push_back(VK_KHR_SURFACE_EXTENSION_NAME);
...
void VulkanRenderer::Impl::FrameRender(ImDrawData* dd)
{
    ...
    VkResult err = vkAcquireNextImageKHR(device, wd->Swapchain, UINT64_MAX, imageAcquired, VK_NULL_HANDLE, &wd->FrameIndex);
    if (err == VK_ERROR_OUT_OF_DATE_KHR || err == VK_SUBOPTIMAL_KHR) swapChainRebuild = true;
    if (err == VK_ERROR_OUT_OF_DATE_KHR) return;
    if (err != VK_SUCCESS && err != VK_SUBOPTIMAL_KHR) { CheckVk(err); return; }
...
void VulkanRenderer::Impl::FramePresent()
{
    ...
    if (err == VK_ERROR_OUT_OF_DATE_KHR || err == VK_SUBOPTIMAL_KHR) { swapChainRebuild = true; return; }
    if (err != VK_SUCCESS) CheckVk(err);
    wd->SemaphoreIndex = (wd->SemaphoreIndex + 1) % wd->SemaphoreCount;
}

// ── Public class ──────────────────────────────────────────────────────────────
VulkanRenderer::VulkanRenderer() : p_(std::make_unique<Impl>()) {}
...
bool VulkanRenderer::BringUp(PlatformBackend* platform, int w, int h)
{
    ...
    if (!p_->CreateInstance()) return false;
    if (!p_->CreateDeviceObjects()) return false;
    if (!p_->CreateDescriptorPool()) return false;
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(p_->physicalDevice, p_->queueFamily, p_->surface, &wsiSupport);
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(
        p_->instance, p_->physicalDevice, p_->device, wd, p_->queueFamily,
        p_->allocator, w, h, p_->minImageCount, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
    ...
    p_->imguiInited = true;
    p_->ready = true;
    std::fprintf(stderr, "[unigui] Vulkan: device + swapchain ready (%dx%d, images=%u), renderer=imgui_impl_vulkan\n",
        w, h, wd->ImageCount);
    return true;
}
...
{
    if (w <= 0 || h <= 0) return;
    if (w == p_->window.Width && h == p_->window.Height) return;
    p_->wantW = w;
    p_->wantH = h;
...
void VulkanRenderer::NewFrameVk()
{
    ...
    if (p_->swapChainRebuild && p_->wantW > 0 && p_->wantH > 0) {
        ...
        ImGui_ImplVulkanH_CreateOrResizeWindow(
            p_->instance, p_->physicalDevice, p_->device, &p_->window, p_->queueFamily,
            p_->allocator, p_->wantW, p_->wantH, p_->minImageCount, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT);
...
{
    if (!p_->ready || !dd) return;
    // Skip rendering to a zero-sized (minimised) swap-chain.
    if (dd->DisplaySize.x <= 0.0f || dd->DisplaySize.y <= 0.0f) return;
    if (p_->swapChainRebuild) return;
    p_->FrameRender(dd);
    p_->FramePresent();
}
...
{
    p_->clearR = r; p_->clearG = g; p_->clearB = b; p_->clearA = a;
}

void VulkanRenderer::Shutdown()
{
    if (!p_->ready) return;
    if (p_->device) vkDeviceWaitIdle(p_->device);
    if (p_->imguiInited) { ImGui_ImplVulkan_Shutdown(); p_->imguiInited = false; }
    if (p_->device) ImGui_ImplVulkanH_DestroyWindow(p_->instance, p_->device, &p_->window, p_->allocator);
    if (p_->descriptorPool) { vkDestroyDescriptorPool(p_->device, p_->descriptorPool, p_->allocator); p_->descriptorPool = VK_NULL_HANDLE; }
    // ImGui_ImplVulkanH_DestroyWindow does not destroy the surface (caller owns it).
    if (p_->surface) { vkDestroySurfaceKHR(p_->instance, p_->surface, p_->allocator); p_->surface = VK_NULL_HANDLE; }
    if (p_->device) { vkDestroyDevice(p_->device, p_->allocator); p_->device = VK_NULL_HANDLE; }
    if (p_->instance) { vkDestroyInstance(p_->instance, p_->allocator); p_->instance = VK_NULL_HANDLE; }
    p_->ready = false;
}

std::unique_ptr<RendererBackend> CreateVulkanRenderer() { return std::make_unique<VulkanRenderer>(); }
