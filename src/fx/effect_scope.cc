...
ShadowEffect::ShadowEffect(float radius, float offsetX, float offsetY, ImU32 color, int samples)
        : radius_(radius)
        , offX_(offsetX)
        , offY_(offsetY)
        , color_(color)
...
void ShadowEffect::Pop() {
    dl_ = nullptr;
}

void ShadowEffect::Draw(ImDrawList* dl) {
    if (radius_ <= 0.f || !hasRect_)
        return;
    ...
    for (int i = 0; i < samples_; ++i) {
        ...
        ImRect expanded(r.Min.x - expand, r.Min.y - expand, r.Max.x + expand, r.Max.y + expand);
...
void GlowEffect::Draw(ImDrawList* dl) {
    ...
    for (int i = 0; i < layers_; ++i) {
        ...
        ImU32 c = (color_ & 0x00FFFFFF) | ((ImU32) ((float) ((color_ >> 24) & 0xFF) * alpha) << 24);
    ...
    for (int i = 0; i < layers; ++i) {
        ...
        for (int dx = -1; dx <= 1; dx += 2) {
            for (int dy = -1; dy <= 1; dy += 2) {
                dl->AddRectFilled(ImVec2(rect_.Min.x + off * dx, rect_.Min.y + off * dy),
                                  ImVec2(rect_.Max.x + off * dx, rect_.Max.y + off * dy), c,
                                  blurRadius_ * 0.5f);
            }
        }
    }
...
void GradientBrush::Horizontal(ImDrawList* dl, const ImVec2& min, const ImVec2& max, ImU32 left,
                               ImU32 right) {
    dl->AddRectFilledMultiColor(min, max, left, right, right, left);
}

void GradientBrush::Vertical(ImDrawList* dl, const ImVec2& min, const ImVec2& max, ImU32 top,
                             ImU32 bottom) {
