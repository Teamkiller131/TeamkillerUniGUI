...
HeroSection::HeroSection(std::string name, std::string title, std::string subtitle)
    : Widget(std::move(name)), title_(std::move(title)), subtitle_(std::move(subtitle)) {}

void HeroSection::SetTitle(std::string t) { title_ = std::move(t); }
void HeroSection::SetSubtitle(std::string t) { subtitle_ = std::move(t); }
void HeroSection::SetBackground(ImU32 top, ImU32 bottom) { bgTop_ = top; bgBottom_ = bottom; }
void HeroSection::SetActionButton(std::string label, std::function<void()> cb) {
    actionLabel_ = std::move(label); actionCallback_ = std::move(cb);
}
void HeroSection::SetHeight(float h) { height_ = h; }
...
void HeroSection::Render() {
    ...
    // ── Gradient background ─────────────────────────────────────────
    fx::GradientBrush::Vertical(dl, cursor, ImVec2(cursor.x + w, cursor.y + height_),
                                 bgTop_, bgBottom_);
    ...
    dl->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 2.f,
                ImVec2(cx - titleSz.x * 0.5f, titleY),
