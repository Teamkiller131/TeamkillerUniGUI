...
#ifdef _WIN32
    NOTIFYICONDATAW nid={};nid.cbSize=sizeof(nid);nid.hWnd=(HWND)hwnd_;nid.uID=1;nid.uFlags=NIF_TIP;
    int tl=(int)title_.size();if(tl>127)tl=127;
    MultiByteToWideChar(CP_UTF8,0,title_.c_str(),tl,nid.szTip,128);nid.szTip[tl]=0;
    Shell_NotifyIconW(NIM_MODIFY,&nid);
#endif
}
void TrayIcon::ShowNotification(std::string title,std::string msg,NotifyType type){
#ifdef _WIN32
    NOTIFYICONDATAW nid={};nid.cbSize=sizeof(nid);nid.hWnd=(HWND)hwnd_;nid.uID=1;nid.uFlags=NIF_INFO;
    switch(type){case NotifyType::Warning:nid.dwInfoFlags=NIIF_WARNING;break;case NotifyType::Error:nid.dwInfoFlags=NIIF_ERROR;break;default:nid.dwInfoFlags=NIIF_INFO;break;}
...
void TrayIcon::BuildContextMenu(HMENU hMenu,std::vector<TrayMenuItem>& items,int& idCounter){
    for(auto& item:items){
        if(item.isSeparator){AppendMenuA(hMenu,MF_SEPARATOR,0,nullptr);continue;}
...
void TrayIcon::ShowContextMenu(){
    ...
    if(cmd>0&&cmd<=idCounter){
        ...
        std::function<TrayMenuItem*(std::vector<TrayMenuItem>&)> find=[&](auto& its)->TrayMenuItem*{
            int curId=0;
            for(auto& it:its){
                if(it.isSeparator)continue;
                if(!it.children.empty()){auto* found=find(it.children);if(found)return found;}
                curId++;if(curId==cmd)return &it;
            }
            return nullptr;
        };
        int curId=0;
        for(auto& it:menu_){
            if(it.isSeparator)continue;
            curId++;if(curId==cmd){if(it.action)it.action();if(onExit_&&it.label=="Exit")onExit_();break;}
        }
    }
    DestroyMenu(hMenu);
}
