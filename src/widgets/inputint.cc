...
void InputInt::Render(){if(!IsVisible())return;int prev=val_; ImGui::PushID(GetName().c_str()); ImGui::InputInt(label_.c_str(),&val_); ImGui::PopID(); if(val_<min_)val_=min_;if(val_>max_)val_=max_; if(val_!=prev&&on_change_)on_change_(val_);}
