...
void Dialog::Render() {
    ...
    if (ImGui::BeginPopupModal(title_.c_str(), &open_)) {
        ...
        okClicked_ = false;
        if (ImGui::Button(okText_.c_str())) { okClicked_ = true; if (onOk_) onOk_(); open_ = false; ImGui::CloseCurrentPopup(); }
...
bool Dialog::IsOpen() const { return open_; }
void Dialog::SetButtons(std::string ok, std::string cancel) { okText_ = std::move(ok); cancelText_ = std::move(cancel); }
void Dialog::SetOnOk(std::function<void()> cb) { onOk_ = std::move(cb); }
void Dialog::SetOnCancel(std::function<void()> cb) { onCancel_ = std::move(cb); }
bool Dialog::WasOkClicked() const { return okClicked_; }
}
