...
ListBox::ListBox(std::string name, std::string label, std::vector<std::string> items, int selected)
    : Widget(std::move(name)), label_(std::move(label)), items_(std::move(items)), selected_(selected), prev_selected_(selected) {}
...
int ListBox::GetSelectedIndex() const { return selected_; }

void ListBox::SetSelectedIndex(int idx) { selected_ = idx; prev_selected_ = idx; }
...
const std::vector<std::string>& ListBox::GetItems() const { return items_; }

void ListBox::SetItems(std::vector<std::string> items) { items_ = std::move(items); }

void ListBox::SetOnChange(std::function<void(int)> cb) { on_change_ = std::move(cb); }
