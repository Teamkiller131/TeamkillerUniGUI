...
void MultiCombo::SetItems(std::vector<std::string> items) {
    items_ = std::move(items);
    selected_.clear();
}
bool MultiCombo::IsSelected(int index) const {
    return selected_.count(index) > 0;
}
void MultiCombo::SetSelected(int index, bool sel) {
    if (sel)
