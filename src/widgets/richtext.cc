...
RichText::RichText(std::string name, std::string text)
    : Widget(std::move(name)), plain_text_(std::move(text)) {
    if (!plain_text_.empty()) spans_.push_back({plain_text_});
}
...
void RichText::Render() {
    ...
    if (spans_.empty()) {
        ...
        for (auto& s : spans_) {
            ...
            if (s.color.w > 0.0f) ImGui::PushStyleColor(ImGuiCol_Text, s.color);
            ImGui::TextUnformatted(s.text.c_str());
            if (s.color.w > 0.0f) ImGui::PopStyleColor();
            if (pushedFont) ImGui::PopFont();
            if (&s != &spans_.back()) ImGui::SameLine(0, 0);
        }
    }
    ImGui::EndGroup();
    ImGui::PopID();
...
std::string RichText::GetText() const { return plain_text_; }

void RichText::SetSpans(std::vector<RichTextSpan> spans) {
    spans_ = std::move(spans);
    plain_text_.clear();
    for (auto& s : spans_) plain_text_ += s.text;
}
