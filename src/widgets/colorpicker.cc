...
void ColorPicker::Render() {
    ...
    if (has_alpha_) {
        ...
        if (ImGui::ColorEdit4(label_.c_str(), c4)) {
            color_ = {c4[0], c4[1], c4[2]};
            color4_ = {c4[0], c4[1], c4[2], c4[3]};
...
void ColorPicker::SetColor(std::array<float, 3> color) {
    color_ = color;
}
void ColorPicker::SetOnChange(std::function<void(std::array<float, 3>)> callback) {
    on_change_ = std::move(callback);
}
void ColorPicker::SetAlpha(bool on) {
    has_alpha_ = on;
}
std::array<float, 4> ColorPicker::GetColorRGBA() const {
    return color4_;
}
void ColorPicker::SetColorRGBA(std::array<float, 4> color) {
    color4_ = color;
