...
void LineEdit::Render() {
    ...
    if (multiline_) {
        changed =
            ImGui::InputTextMultiline(label_.c_str(), buffer_, max_length_, ImVec2(-1, 80), flags);
    } else {
        changed =
...
void LineEdit::Undo() {
    if (undoIndex_ > 0) {
        undoIndex_--;
        value_ = undoStack_[undoIndex_];
        std::copy_n(value_.data(), std::min(value_.size(), sizeof(buffer_) - 1), buffer_);
        buffer_[std::min(value_.size(), sizeof(buffer_) - 1)] = 0;
    }
}
void LineEdit::Redo() {
    if (undoIndex_ + 1 < (int) undoStack_.size()) {
...
void LineEdit::SetValidator(std::function<bool(const std::string&)> fn) {
    validator_ = std::move(fn);
}
bool LineEdit::HasError() const {
    return has_error_;
}
void LineEdit::SetPasswordMode(bool on) {
    password_ = on;
}
void LineEdit::SetMultiline(bool on) {
    multiline_ = on;
}
