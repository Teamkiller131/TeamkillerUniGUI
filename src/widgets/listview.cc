    ...
    for (int i = 0; i < (int)items_.size(); i++) {
        ...
        if (ImGui::Selectable(items_[i].c_str(), sel)) {
            if (multiSelect_) {
                ...
                if(it!=multiSelected_.end()) multiSelected_.erase(it); else multiSelected_.push_back(i);
...
void ListView::SetMultiSelect(bool on) { multiSelect_ = on; }
std::vector<int> ListView::GetSelectedItems() const { return multiSelect_ ? multiSelected_ : (selected_>=0?std::vector<int>{selected_}:std::vector<int>{}); }
}
