...
static bool BringUpBackend(BackendType type, const AppConfig& config, float& dpiOut){
    ...
    g_backend=type; g_platform=std::move(be.platform); g_renderer=std::move(be.renderer);

    if(!g_platform||!g_platform->Init(nullptr)){
        UNIGUI_LOG_ERROR("Platform init failed (backend={})",(int)type);ResetBackendOnly();return false;}
    ...
    if(dpi<=0){dpi=DetectDPIScale(g_platform->GetWindowHandle()); if(dpi<0.5f)dpi=1.0f;}
    ...
    if(type==BackendType::DX11){
        ...
        if(!CreateDX11DeviceAndSwapChain(hwnd,pw,ph,&dev,&ctx,&swap,&rtv)){
            UNIGUI_LOG_ERROR("DX11 device/swapchain creation failed");ResetBackendOnly();return false;}
    ...
    if(type==BackendType::DX12){
        ...
        if(!CreateDX12DeviceAndSwapChain(hwnd,pw,ph,&dev,&queue,&cmdList,&swap,&rtvHeap,&srvHeap)){
            UNIGUI_LOG_ERROR("DX12 device/swapchain creation failed");ResetBackendOnly();return false;}
    ...
    if(type==BackendType::Vulkan
       ...
       ||type==BackendType::SDL3_Vulkan
        ...
        int pw=0,ph=0; g_platform->GetClientSize(&pw,&ph);
        if(pw<=0){pw=config.width;ph=config.height;}
        ...
        if(!vr->BringUp(g_platform.get(),pw,ph)){
            UNIGUI_LOG_ERROR("Vulkan device/swapchain creation failed");ResetBackendOnly();return false;}
    }
#endif

    if(!g_renderer||!g_renderer->Init(ImGui::GetCurrentContext())){
        UNIGUI_LOG_ERROR("Renderer init failed (backend={})",(int)type);ResetBackendOnly();return false;}
    {
        const char* rn=ImGui::GetIO().BackendRendererName;
        const char* pn=ImGui::GetIO().BackendPlatformName;
        UNIGUI_LOG_INFO("Active backend: renderer={} platform={}",rn?rn:"(none)",pn?pn:"(none)");
    }
    ...
    auto& io=ImGui::GetIO();
    ThemeConfig tc=config.theme; tc.dpi_scale=dpi; ApplyTheme(tc);
...
bool Init(const AppConfig& config){
    ...
    if(!ok && config.backend!=BackendType::GLFW_GL3){
        UNIGUI_LOG_WARN("Backend {} unavailable — falling back to GLFW/OpenGL3",(int)config.backend);
...
bool NewFrame(){
...
#endif
#ifdef UNIGUI_HAS_DX12
    if(g_backend==BackendType::DX12)ImGui_ImplDX12_NewFrame();
#endif
#ifdef UNIGUI_HAS_VULKAN
    if(g_backend==BackendType::Vulkan
       ...
       ||g_backend==BackendType::SDL3_Vulkan
        ...
        int cw=0,ch=0; g_platform->GetClientSize(&cw,&ch);
        auto* vr=static_cast<VulkanRenderer*>(g_renderer.get());
        if(cw>0&&ch>0)vr->RequestResize(cw,ch);
        vr->NewFrameVk();
    }
#endif
