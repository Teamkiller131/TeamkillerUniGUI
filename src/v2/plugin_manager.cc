...
IPlugin* Manager::Load(const std::string& path) {
    ...
    if (!h) { UNIGUI_LOG_ERROR("Failed to load: {}", path); return nullptr; }

    auto cf = (CreatePluginFn)GetSymOS(h, "CreatePlugin");
    auto df = (DestroyPluginFn)GetSymOS(h, "DestroyPlugin");
    if (!cf || !df) { UNIGUI_LOG_ERROR("Missing exports: {}", path); FreeLibOS(h); return nullptr; }

    IPlugin* raw = cf();
    if (!raw) { UNIGUI_LOG_ERROR("CreatePlugin returned null: {}", path); FreeLibOS(h); return nullptr; }
    if (!raw->Init()) { UNIGUI_LOG_ERROR("Init failed: {}", path); df(raw); FreeLibOS(h); return nullptr; }

    auto info = raw->GetInfo();
    LoadedPlugin lp;
    lp.path = path; lp.name = info.name; lp.handle = h; lp.raw = raw;
    lp.ptr = std::unique_ptr<IPlugin, std::function<void(IPlugin*)>>(raw, [df,h](IPlugin* p){ df(p); FreeLibOS(h); });
...
IPlugin* Manager::Register(IPlugin* plugin) {
    ...
    LoadedPlugin lp;
    lp.name = info.name; lp.raw = plugin;
    lp.ptr = std::unique_ptr<IPlugin, std::function<void(IPlugin*)>>(plugin, [](IPlugin* p){ delete p; });
