...
void Database::BindParams(sqlite3_stmt* stmt, const std::vector<Param>& params) {
    for (int i = 0; i < (int)params.size(); i++) {
        int idx = i + 1;
        auto& p = params[i];
        if (std::holds_alternative<int>(p)) sqlite3_bind_int(stmt, idx, std::get<int>(p));
        else if (std::holds_alternative<double>(p)) sqlite3_bind_double(stmt, idx, std::get<double>(p));
