...
TEST_F(ScrollAreaTest, Render_DoesNotCrash) { unigui::ScrollArea sa("sa"); sa.Render(); }
TEST_F(ScrollAreaTest, ContentCallback_IsCalled) { unigui::ScrollArea sa("sa"); bool called=false; sa.SetContentCallback([&](){called=true;}); sa.Render(); EXPECT_TRUE(called); }
