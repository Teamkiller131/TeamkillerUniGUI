...
TEST_F(ColorPickerTest, Render_DoesNotCrash) { unigui::ColorPicker cp("cp","Color"); cp.Render(); }
TEST_F(ColorPickerTest, GetColor_Defaults) { unigui::ColorPicker cp("cp","Color"); auto c = cp.GetColor(); EXPECT_FLOAT_EQ(c[0], 0.0f); }
